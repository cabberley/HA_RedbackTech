    ...
    for entity_key in entity_keys:
        if entity_key[7:] in ENTITY_DETAILS:
            binary_sensors.extend(
                [RedbackTechBinarySensorEntity(coordinator, entity_key)]
            )
...
class RedbackTechBinarySensorEntity(CoordinatorEntity, BinarySensorEntity):
    ...
    async def create_schedule_event(
        self, power_mode, power_setting, duration, start_time
    ):
        """Update the current value."""
        await self.coordinator.client.create_schedule_event(
            self.ent_id,
            power_setting_mode=power_mode,
            power_setting_watts=power_setting,
            power_setting_duration=duration,
