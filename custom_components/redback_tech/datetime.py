...
from .datetime_properties import ENTITY_DETAILS
...
async def async_setup_entry(
    ...
    for entity_key in entity_keys:
        if entity_key[7:] in ENTITY_DETAILS:
            datetime_entitys.extend(
                [RedbackTechDateTimeEntity(coordinator, entity_key)]
            )
    ...
    async def async_set_value(self, value: datetime) -> None:
        ...
        if self.ent_key[4:7] == "inv":
            await self.coordinator.client.update_inverter_control_values(
                self.ent_data.data["device_id"], "start_time", value
            )
            ...
            if self.ent_data.data["entity_name"] == "op_env_create_start_time":
                await self.coordinator.client.update_op_envelope_values(
                    self.ent_data.data["device_id"], "StartAtUtc", value
                )
            elif self.ent_data.data["entity_name"] == "op_env_create_end_time":
                await self.coordinator.client.update_op_envelope_values(
                    self.ent_data.data["device_id"], "EndAtUtc", value
                )
