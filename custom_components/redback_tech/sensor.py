...
async def async_setup_entry(
    ...
    LOGGER.debug(f'Energy Key Match: {entity_keys}')
    for entity_key in entity_keys:
        LOGGER.debug(f'Energy Key Match: {entity_key}')
        if entity_key[7:] in  ENTITY_DETAILS: 
            #if ENTITY_DETAILS[entity_key[7:]]['state_class'] is not None: #          CLASSIFY_ENERGY:
...
class RedbackTechSensorEntity(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, entity_key):
        ...
        self.entity_id = 'sensor.rb'+self.ent_id[:4] + '_' + self.ent_id[-3:].lower() + '_' + ENTITY_DETAILS[self.ent_key[7:]]['name']
        LOGGER.debug(f'entity_data1: {self.ent_data}')
        LOGGER.debug(f'entity_data2: {self.ent_id}')

    @property
    def ent_data(self) -> Inverters:
        """Handle coordinator data for entities."""
        LOGGER.debug(f'entity_data22: {self.coordinator.data.entities[self.ent_key]}')
    ...
    def native_value(self) -> float:
        ...
        if ENTITY_DETAILS[self.ent_key[7:]]['name']  == 'Timestamp Utc':
            value = datetime.fromisoformat(self.ent_data.data['value']).replace(tzinfo=timezone.utc).astimezone(timezone.utc).isoformat()
        else:
