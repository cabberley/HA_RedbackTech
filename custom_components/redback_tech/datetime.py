...
from .datetime_inverter_properties import (
    ENTITY_DETAILS
)
...
async def async_setup_entry(
    ...
    LOGGER.debug(f'Datetime Key Match: {entity_keys}')
    for entity_key in entity_keys:
        LOGGER.debug(f'Datetime Key Match: {entity_key}')
...
class RedbackTechDateTimeEntity(CoordinatorEntity, DateTimeEntity):
    ...
    def __init__(self, coordinator, entity_key):
        ...
        LOGGER.debug(f'datetime_data1: {self.ent_data}')
        LOGGER.debug(f'datetime_data2: {self.ent_id}')

    @property
    def ent_data(self) -> ScheduleDateTime:
        """Handle coordinator data for entities."""
        LOGGER.debug(f'datetime_data22: {self.coordinator.data.schedules_datetime_data[self.ent_key]}')
    ...
    async def async_set_value(self, value: datetime) -> None:
        ...
        LOGGER.debug(f'datetime_data: {value}')
        LOGGER.debug(f'datetime_data: {self.ent_data.data['value']}')
